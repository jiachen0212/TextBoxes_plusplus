...
def txtToXml(image_path, txt_path):
    for txt_file in tqdm(glob.glob(txt_path + '/*.txt')):
        ...
        for i, line in enumerate(root):
            ...
            xmin.text = min(column[0], column[2], column[4], column[6])
            ymin = SubElement(node4, 'ymin')
            ymin.text = min(column[1], column[3], column[5], column[7])
            xmax = SubElement(node4, 'xmax')
            xmax.text = max(column[0], column[2], column[4], column[6])
            ymax = SubElement(node4, 'ymax')
            ymax.text = max(column[1], column[3], column[5], column[7])
